...
const createServer = () => {
  ...
  app.delete('/things/1', deleting);
  app.get('/*', errorhandle);

  function gettingstarted(request,respond){
    respond.status(200).send('No');
  }
  function deleting(request,respond){
    respond.status(405).send('Way');
  }
  function errorhandle(request,respond){
    respond.status(404).send('This is right');
  }
...
let biggerThanLuke = (arr) => {
  ...
  arr.filter((element)=>{
    ...
    if(Number(element.mass) > Number(lukemass)){
...
const sortBy = (property, arr) => {
  // Solution code here...
if(property==='name'){
return arr.sort(function(a, b){return a.name.charCodeAt(0) - b.name.charCodeAt(0)});
}
if(property==='price'){
