...
Without altering the html, write a function named generateSubmitButton that uses jQuery to create a submit button
with the text "submit" and append it to the DOM.
------------------------------------------------------------------------------------------------ */
...
const citiesAtoJ = (arr) => {
  ...
  function myfunction(wor){
   let regex = /^[A-J].*\w/gm;
   let result = wor.match(regex);
  
    if(result !== null){
     final.push(result.toString());
    }
  }
