...
const createServer = () => {
  ...
  var server = app.listen(3301, function () {
    ...
    app.get('/favoritefoods', handelfavoritefoods);
    app.get('/*',handelError);
  ...
  function handleHello(req,res){
    res.status(200).send('greeting of my choice');
  };

  function handelaboutme(req,res){
    res.status(200).send('short bio about me');

  };
  
  function handelfavoritefoods(req,res){
    res.status(200).send(['mansaf','shakreiah','pizza']);
...
const totalSum = (input) => {
  // Solution code here...
 let result = input.reduce(function (a,n){
  let smallArrSum = n.reduce(function(h,z){
    h+=z;
    return h;
  },0);
  a+=smallArrSum;
